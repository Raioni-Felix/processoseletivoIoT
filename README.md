...
- **Máquina de Estados LDR:** Avalia continuamente o canal ADC. Transita entre "Linha Livre" (`ADC >= LIMIAR_BLOQUEIO`, alta luminosidade) e "Peça Bloqueando" (`ADC < LIMIAR_BLOQUEIO`, baixa luminosidade). A transição para bloqueio inicia o cronômetro de micro-parada; a transição de volta para linha livre consolida o incremento na contagem.
...
- **Configuração do ADC e Limiares:** A configuração da atenuação do LDR para 11dB (`ATTN_11DB`) expande o limite de leitura do conversor analógico-digital para cerca de 3.3V (tensão de operação do ESP32), evitando saturação do sinal luminoso. A resolução parametrizada em 12 bits (`WIDTH_12BIT`) provê um mapeamento de 0 a 4095. O módulo do LDR forma um divisor de tensão entre o fotoresistor e um resistor fixo, de modo que mais luz incidente resulta em maior leitura de ADC. Isso permite estabelecer um limiar lógico único e bem definido (`LIMIAR_BLOQUEIO = 2500`) que distingue o estado de esteira livre (alta luminosidade, `ADC >= 2500`) do estado de objeto bloqueando o feixe (baixa luminosidade, `ADC < 2500`), sem necessidade de uma banda de histerese entre dois limiares distintos.
